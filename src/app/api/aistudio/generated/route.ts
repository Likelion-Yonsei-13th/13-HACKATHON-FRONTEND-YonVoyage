// src/app/api/aistudio/generated/route.ts
...
export async function GET(req: Request) {
  ...
  try {
    const upstream = await fetch(`${UPSTREAM_BASE}/api/studio/generated/${q}`, {
      cache: "no-store",
    });
    ...
    list = (Array.isArray(list) ? list : []).map((it: any) => {
      const media = it?.url ?? it?.generated_image;
    ...
    console.error("[aistudio/generated] proxy error:", e?.message || e);
